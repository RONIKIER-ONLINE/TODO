...
public class TaskServiceGraph implements TaskService, DialogService {
    ...
    public List<Task> allTasks(SortOrder sortOrder) {
        ...
        switch (sortOrder) {
            case NONE:
                return StreamSupport.stream(taskRepository.findAll().spliterator(), true).collect(Collectors.toList());

            case PRIORITY:
                taskComparator = (Task taskA, Task taskB) -> {

                    int stateDelta = Arrays.asList(TaskState.values()).indexOf(taskA.getTaskState()) - Arrays.asList(TaskState.values()).indexOf(taskB.getTaskState());
                    int statusDelta = Arrays.asList(TaskStatus.values()).indexOf(taskA.getTaskStatus()) - Arrays.asList(TaskStatus.values()).indexOf(taskB.getTaskStatus());
